...
var SendArgs_Req_DEFAULT *email.SendReq

func (p *SendArgs) GetReq() *email.SendReq {
	if !p.IsSetReq() {
...
type SendResult struct {
	Success *email.SendResp
}

var SendResult_Success_DEFAULT *email.SendResp

func (p *SendResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(email.SendResp)
	}
...
func (p *SendResult) GetSuccess() *email.SendResp {
	if !p.IsSetSuccess() {
...
func (p *kClient) Send(ctx context.Context, Req *email.SendReq) (r *email.SendResp, err error) {
	var _args SendArgs
	_args.Req = Req
